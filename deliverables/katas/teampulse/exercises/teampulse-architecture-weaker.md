...
The system is built on a modern three-tier architecture using React (frontend), Node.js (backend), and PostgreSQL (database), with Entra ID/OIDC for authentication. The architecture emphasizes privacy-by-design, scalability, and performance to meet the non-functional requirements specified in the PRD.
...
2. **Separation of Concerns**: Manager-facing and engineer-facing flows are architecturally isolated to prevent data leakage.
3. **Stateless Backend**: Enables horizontal scaling and simplifies deployment.
4. **Event-Driven Notifications**: Decoupled notification delivery for reliability and scalability.
...
│  └──────────────┘  └──────────────┘  └──────────────────┘  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────┐  │
│  │ Config API   │  │ Notification │  │ Data Purge Job   │  │
│  │ (Manager)    │  │ Service      │  │ (Scheduled)      │  │
│  └──────────────┘  └──────────────┘  └──────────────────┘  │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
...
graph TB
    ...
    subgraph "Application Services"
        ...
        JobSvc["Job Service<br/>- Scheduled surveys<br/>- Data purge<br/>- Cleanup tasks"]
    end
...
graph TB
    ...
    subgraph "Notification Service"
        ...
        RetryLogic["Retry Manager<br/>- Exponential backoff<br/>- Max retries<br/>- Dead letter queue"]
    end
    
    subgraph "Job Service"
        Scheduler["Job Scheduler<br/>- Cron triggers<br/>- Survey period mgmt<br/>- Data purge"]
        SurveyPeriodMgr["Survey Period Manager<br/>- Create periods<br/>- Close periods<br/>- Trigger notifications"]
        DataPurge["Data Purge Job<br/>- Delete old responses<br/>- Archive audit logs"]
    end
...
erDiagram
    ...
    SURVEY_CONFIG {
        ...
        int day_of_week
        string send_time
        string notification_channel "email|slack"
        timestamp created_at
        timestamp updated_at
    }
...
1. **No PII in Responses Table**: The `RESPONSE` table contains only:
   - `survey_period_id` (which team, which period)
   - `question_id` (which question)
   - `answer_value` (the response)
   - `submitted_at` (timestamp)
   
   **No columns for**: user_id, email, name, entra_id, or any identifier.

2. **Separate Notification Tracking**: The `NOTIFICATION` table tracks delivery status and tokens, but is never joined with `RESPONSE` in queries.

3. **Aggregation-Only Queries**: All manager-facing queries aggregate responses at the (team_id, survey_period_id, question_id) level. Individual response rows are never exposed.

4. **Audit Logging**: Configuration changes are logged with manager identity, but survey responses are never logged with any identifier.
...
## 9. Conceptual Database Design Details

### 9.1 Anonymity Enforcement

**Response Storage:**
```sql
CREATE TABLE responses (
    id UUID PRIMARY KEY,
    survey_period_id UUID NOT NULL REFERENCES survey_periods(id),
    question_id UUID NOT NULL REFERENCES questions(id),
    answer_value VARCHAR(500) NOT NULL,
    submitted_at TIMESTAMP NOT NULL DEFAULT NOW(),
    
    -- NO user_id, email, name, entra_id, or any PII
    -- Uniqueness constraint prevents duplicate submissions
    UNIQUE(survey_period_id, question_id, answer_value, submitted_at)
);
```
...
SELECT 
    ...
    COUNT(DISTINCT sp.id) as response_count
FROM responses r
...
### 9.2 Notification Token Design

**Token Generation:**
```
Token = HMAC-SHA256(
  secret_key,
  survey_period_id + team_member_id + timestamp
)
```

**Token Validation:**
- Verify HMAC signature
- Check expiry (7 days from period start)
- Verify not already used (mark as used on first submission)
- Verify survey period is still active
...
| **Cross-team data access** | All queries filtered by authenticated manager's team_id |
...
- Simplifies privacy compliance (GDPR, etc.)
...
The architecture is ready for implementation by a development team using React, Node.js, and PostgreSQL as specified in the PRD.
...
|---------|------|--------|---------|
| 1.0 | 2026-06-28 | Senior Solutions Architect | Initial architecture document |
